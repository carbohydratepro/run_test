...
def shadow_detection(image_file, shadow_mask_file, convolve_window_size=5, num_thresholds=3, struc_elem_size=5):
    ...
    if (convolve_window_size % 2 == 0):
        raise ValueError(
            'Please make sure that convolve_window_size is an odd integer')

    buffer = int((convolve_window_size - 1) / 2)

    with rasterio.open(image_file) as f:
        metadata = f.profile
        img = rescale_intensity(np.transpose(
            f.read(tuple(np.arange(metadata['count']) + 1)), [1, 2, 0]), out_range='uint8')
        img = img[:, :, 0: 3]
    ...
    avg_kernel = np.ones(
        (convolve_window_size, convolve_window_size)) / (convolve_window_size ** 2)
    ...
    labels = KMeans(n_clusters=num_thresholds + 1,
                    max_iter=10000).fit(flattened_sr_img).labels_
    ...
    shadow_mask = np.expand_dims(np.uint8(cv2.morphologyEx(
        shadow_mask_initial, cv2.MORPH_CLOSE, struc_elem)), axis=0)
...
def shadow_correction(image_file, shadow_mask_file, corrected_image_file, exponent=1):
    ...
    for i in range(img.shape[2]):
        ...
        shadow_stats = np.float32(np.mean(
            ((shadow_area_mask ** exponent) / np.sum(shadow_mask))) ** (1 / exponent))
        non_shadow_stats = np.float32(np.mean(
            ((non_shadow_area_mask ** exponent) / np.sum(non_shadow_mask))) ** (1 / exponent))
        mul_ratio = ((non_shadow_stats - shadow_stats) / shadow_stats) + 1
        corrected_img[:, :, i] = np.uint8(
            non_shadow_area_mask + np.clip(shadow_area_mask * mul_ratio, 0, 255))
...
def main():
    shadow_correction("./Shadow-Detection-Algorithm-for-Aerial-and-Satellite-Images/Test_Image.JPG", "./Shadow-Detection-Algorithm-for-Aerial-and-Satellite-Images/Shadow_Mask_image.JPG", "./Shadow-Detection-Algorithm-for-Aerial-and-Satellite-Images/output.jpg")

if __name__ == "__main__":
    # print(os.path.exists("Test_Image.JPG"))
    main()